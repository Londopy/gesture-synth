...
- `scripts/stage-check.mjs`: headless Stage regression check (menu, Sets wheel, a set on the beat rated Flawless, a set 150 ms late rated Rough with Late judgments, local records).
- Community service: Stage boards (`POST /scores`, `GET /scores?song=`), best set per player per song, rehearsal sets refused; the schema gains a `scores` table applied on startup.
